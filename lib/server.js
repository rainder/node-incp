...
module.exports = class Server {
  ...
  _onConnection(socket) {
    ...
    socket.on('data', dataHandler.handle((message) => this._onRequest(socket, ICRequest.createFromIncoming(message))));
  ...
  listen() {
    ...
    return co(function *() {
      yield cb => self[SERVER$].listen(options, cb);
      const address = self[SERVER$].address();
      
      d(`Server is listening on ${address.address}:${address.port}`);
