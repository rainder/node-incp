...
const endpoints = {
  request: {
    ...
    introduce: require('./endpoint/request/introduce'),
...
const PROTO = new Buffer([0x48, 0xfa, 0x55, 0x10]);
...
module.exports = class DataHandler extends RequestResponseWrapper {
  ...
  init(connection) {
    ...
    connection.on('data', (data) => {
      ...
      while (stack.length) {
        ...
        this.incomingMessage(payload, connection);
      }
    });
  }
  ...
  onRequest(data, connection) {
    ...
    function *go() {
      ...
      return yield endpoints.request[json.endpoint](json.data, this.incp, connection);
    }
  }
  ...
  onPush(data, connection) {
    ...
    function *go() {
      ...
      return yield endpoints.push[json.endpoint](json.data, this.incp, connection);
    }
  }
};
