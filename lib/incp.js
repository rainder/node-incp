...
class INCP extends events.EventEmitter {
  ...
  _introduce(newNode) {
    for (const oldNode of this[$NODES].values()) {
      ...
      // d.introducing(`${oldNode.getConfiguration().getId()} to ${newNode.getConfiguration().getId()}`);
  ...
  connectTo(port, host = '127.0.0.1', options = {}) {
    ...
    return co(function *() {
      self._checkBlacklist(port, host);

      for (const node of self.getNodes().values()) {
      ...
      const response = yield client.getSocket().dataHandler.request({
        ...
        if (e.type === 'INCPError') {
          if (e.errno === INCPError.LOOPBACK_CONNECTION.errno()) {
            self.blacklist.add(`${host}:${port}`);
          }
        }
      ...
      if (self[$NODES].has(remoteConfiguration.getId())) {
        client.close();
