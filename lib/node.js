...
module.exports = class Node extends events.EventEmitter {
   ...
   */
  getClientSocket() {
    if (!this.isClientSocketAlive()) {
      throw new Error('Client socket is not writable');
    }
  ...
  sendRequest(data) {
    return new Promise((resolve, reject) => {
      dataHandler.req(this.getClientSocket(), 'external', data).then(resolve, reject);
    });
  }
  ...
  sendPush(data) {
    return new Promise((resolve, reject) => {
      dataHandler.push(this.getClientSocket(), 'external', data).then(resolve, reject);
    });
  }
