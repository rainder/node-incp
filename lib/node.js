...
module.exports = class Node {
  constructor(host, port) {
    ...
    socket.on('end', (err) => this._onEnd(err));
   ...
   */
  _onClose(hadError) {
    d(`INCP Node connection closed ${hadError}: ${this.id}`);
   ...
   */
  _reconnect() {
    d('reconnect attempt no', this.connectionFailureCount);
   ...
   */
  _onConnect() {
    d(`INCP Node connection established ${this.id}`);
    this.connectionFailureCount = 0;
  ...
  _getReconnectTimeout(failureCount) {
    return Math.min(5000, Math.pow(failureCount, 2) * 2 + 100);
  }
  ...
  _onMessage(socket, message) {
    console.error(`unhandled node message`, message);
  }
